      ...
      <div class="right">
        ...
        <div class="peopledataecharts">
          ...
          <el-radio-group  class="choose" v-model="radio" size="small">
            <router-link to="/realtime/car-day"><el-radio-button label="月度"></el-radio-button></router-link>
...
export default {
  ...
  methods:{
    ...
    drawChartYear(){
      let data={year:2019};
      getCarDataYear(data).then(res=>{
        // console.log(res.data)
        this.inQuantity = []; // 每月进入车辆总数
        this.month = [];      // 月份
        this.outQuantity = [];// 每月离开车辆总数
        for(let i=0; i<res.data.length; i++){
          this.inQuantity.push(res.data[i].traffic_inQuantity);
          this.month.push(res.data[i].traffic_month);
          this.outQuantity.push(res.data[i].traffic_outQuantity);
        }
        // console.log(this.inQuantity)
  ...
  mounted() {
    // this.getDayNow();        // 获取当前日期
    this.getCarDay();         // 当日车流量总数
    this.drawChartYear();    // 月度车流数据统计图
